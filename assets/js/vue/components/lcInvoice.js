            ...
            <div id="invoiceContent">
                ...
                <div class="row">
                    <div class="col-xs-5 m-auto">
                        <h5 style="margin-bottom:4px; margin-top:25px; text-align:center; font-weight:bold;"> Expenses Details</h5>
                        <table class="pull-left" _a584de>
                            ...
                            <tbody> 
                                ...
                                <tr> 
                                    <td style="font-weight:bold;"> Total Expense Bill: </td>
                                    <td style="font-weight:bold;"> {{ cart.reduce((prev,curr) => {return prev + parseFloat(curr.pay_bill)}, 0) }} </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>

                      <div class="col-xs-7 m-auto">
                            ...
                            <tbody> 
                                <tr v-for="lc in productCart"> 
                                     ...
                                     <td> {{lc.LC_PurchaseDetails_Rate}}</td>
                                      ...
                                      <td> {{lc.LC_PurchaseDetails_TotalAmount}}</td>
                                </tr>
                                <tr> 
                                  <td colspan="3" style="font-weight:bold;"> Total Purchase Amount </td>
                                  <td style="font-weight:bold;"> {{ productCart.reduce((prev,curr) => {return prev + parseFloat(curr.LC_PurchaseDetails_TotalAmount)}, 0) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
  ...
  methods: {
    getSales() {
      axios.post("/get_lc_details", { lcId: this.lc_id }).then((res) => {
        ...
        console.log(this.lc.branch_name);
      });
    },

    getLCPurchaseDetails() {
      axios
        .post("/get_lc_purchase_details", { lcId: this.lc_id })
        .then((res) => {
    ...
    // getExpenseDetails() {
    //   axios
    //     .post("/get_expense_details", { lcId: this.sales_id })
    //     .then((res) => {
    //       this.sales = res.data;
    //     });
    // },
